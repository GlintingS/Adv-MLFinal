...
try:
    ...
    logger.info("Data loaded from disk (%d rows)", len(df))
except Exception as exc:
    logger.warning("Could not load data from disk: %s", exc)
...
if df is None:
    ...
    if uploaded is not None:
        ...
        logger.info("Data loaded from upload (%d rows)", len(df))
...
with tab_elbow:
    ...
    try:
        ...
        logger.error("Elbow tab error: %s", exc)
        st.error(f"Error computing Elbow method: {exc}")
...
with tab_sil:
    ...
    try:
        ...
        logger.error("Silhouette tab error: %s", exc)
        st.error(f"Error computing Silhouette scores: {exc}")
...
with tab_clusters:
    ...
    try:
        ...
        logger.error("Clusters tab error: %s", exc)
        st.error(f"Error during clustering: {exc}")
